...
class QueryResult:
    ...
    family_selection_trace: pd.DataFrame
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--family-exploration-limit", type=int, default=5, help="family_selection 等距探索候选上限，默认 5")
...
def build_final_suggested_bill(
    ...
    family_map = {cell_text(row.get("family_id")): row for _index, row in candidate_families.iterrows()}
    quantity_map = {cell_text(row.get("family_id")): row for _index, row in quantity_decisions.iterrows()}
    rows: list[dict[str, Any]] = []
    for index, selected in selected_families.iterrows():
        family_id = cell_text(selected.get("family_id"))
        family = family_map.get(family_id)
        decision = quantity_map.get(family_id)
        ...
        row = {
            ...
            "清单项名称": cell_text(family.get("representative_cost_item_name")),
            "项目特征/施工工艺": cell_text(family.get("representative_project_description")),
            ...
            "推荐依据": cell_text(selected.get("selection_reason")),
...
def display_item_label(row: pd.Series) -> str:
    name = cell_text(row.get("清单项名称"))
    description = short_description(row.get("项目特征/施工工艺"))
...
def build_parse_info(
    ...
    family_selection_input_count: int,
    family_selection_selected_count: int,
    family_selection_trace: dict[str, Any],
    ...
    rows = [
        ...
        ("family_selection_input_count", family_selection_input_count),
        ("family_selection_candidate_count", len(family_selection_meta.get("candidate_ids") or [])),
        ("family_selection_candidate_ids", json_text(family_selection_meta.get("candidate_ids") or [])),
        ("family_selection_selected_count", family_selection_selected_count),
        ("family_selection_selected_ids", json_text(family_selection_meta.get("selected_ids") or [])),
        ("family_selection_selected_detail", json_text(family_selection_meta.get("selected_detail") or [])),
        ("family_selection_candidate_source_counts", json_text(family_selection_meta.get("candidate_source_counts") or {})),
        ("family_selection_exploration_count", family_selection_meta.get("exploration_count", "")),
        ...
        ("invalid_family_ids", join_non_empty([*(family_selection_meta.get("invalid_family_ids") or []), *(quantity_decision_meta.get("invalid_family_ids") or [])])),
        ("duplicate_family_ids", join_non_empty([*(family_selection_meta.get("duplicate_family_ids") or []), *(quantity_decision_meta.get("duplicate_family_ids") or [])])),
...
def write_query_result_workbook(output_path: Path, result: QueryResult, display: bool = False) -> None:
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        display_frame(result.family_selection_trace, display).to_excel(
            writer,
            sheet_name="family_selection_trace",
            index=False,
        )
...
def run_query(
    ...
    if warnings:
        append_trace_warnings(family_selection_trace, warnings)
        append_trace_warnings(dedup_selection_trace, warnings)
        append_trace_warnings(quantity_decision_trace, warnings)
    families_for_llm_count = len(family_selection_meta.get("candidate_ids") or [])
    ...
    result = QueryResult(
        ...
        family_selection_trace=family_selection_trace_frame,

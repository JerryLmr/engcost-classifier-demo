...
- `option_evidence_expansion`：展示每个最终清单的价格证据 family 和样本数在扩充前后的变化。扩充只对原 practice option 去重价格证据少于 10 条的最终项执行；LLM 仅为代表 family 与最多 20 个同单位候选提取维修对象/动作、厚度、材料和楼层/高度标签，程序仅在单位及四项标签字符串完全一致时加入候选。原证据不少于 10 条或没有候选时不调用 LLM，仍保留 sheet 与 trace 记录。
